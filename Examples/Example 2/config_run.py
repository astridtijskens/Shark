...
simulated. Next, the output files are read and postprocessed (see postproc_convergence.py)
...
scenario = {'parameter':'brick material', 'value':['Brick1','Brick2','Brick3']} # currently, only one scenario parameter is supported
...
def main():
    ...
    while conv == False:
        ...
        for r in range(seq):
            print('Sequence %i' %r)
            # Generate samples
            samples_all = sampling.main(scenario=scenario, dist=distributions, runs=used_samples_per_set+new_samples_per_set, sets=1, strat='sobol convergence', path=path['samples'], seq=r)
            samples_new = samples_all[used_samples_per_set:]

            # Generate Delphin files
            if buildcomp != None:
                autovar.main(path, samples_new, buildcomp={'component':buildcomp, 'cell':buildcomp_elem, 'dir': dir_cr},
                             number_of_years=number_of_years, intclimType=interior_climate_type, simulRain=simulate_wdrain, seq=r, start_num=used_samples_per_set, feedback=False)
            else:
                autovar.main(path, samples_new, number_of_years=number_of_years, intclimType=interior_climate_type, simulRain=simulate_wdrain,
