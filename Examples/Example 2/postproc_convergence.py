...
def calc_heatloss(X):
    ...
    dQ = abs(Q[-1]-Q[0])
        # substract last heatflux Q[-1] from first heatflux Q[0] to get total heat loss over whole period (defined by output grid)
    return dQ
...
def calc_frost(X):
    ...
    T = list(X['temperature'][:,1])[1:]
        ...
        # [:,1] means we select the second column of the output file (note that python indexing starts from 0) - see output file for which column is needed
        # [1:] means we exclude the first output time step - this depends on the way the output files and output grids are defined
    ...
    W = list(X['moistmass'][:,0])[1:]
        ...
        # [:,0] means we select the first column of the output file (note that python numbering starts from 0) - see output file for which column is needed
        # [1:] means we exclude the first output time step - this depends on the way the output files and output grids are defined
